...
public class ReportModel extends Model{
    ...
    public String EXPENSEDAY = "SELECT bill_restock_ingredient.id as id, ingredients.name as name, sum(bill_restock_ingredient.quantity) as quantity, sum(bill_restock_ingredient.subvalue) as subvalue, strftime(\"%Y-%m-%d\", bill_restock_ingredient.created_at) as updated_at from bill_restock_ingredient inner join ingredients on ingredients.id = bill_restock_ingredient.ingredient_id GROUP by updated_at,name ORDER by updated_at";
    ...
    public String SELLDAY = "SELECT bills_products.id as id, products.name as name,sum(bills_products.quantity) as quantity, sum(bills_products.subvalue) as subvalue, strftime(\"%Y-%m-%d\", bills_products.created_at) as updated_at from bills_products inner join products on products.id = bills_products.product_id GROUP by updated_at,name ORDER BY updated_at";
